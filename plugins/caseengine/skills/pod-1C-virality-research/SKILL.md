...
description: >
  ...
  hooks for this episode", or "/pod-1C-virality-research". Research Step 1C of the
  podcast pipeline - OPTIONAL. Runs in tandem with pod-1A-entity-research and
  pod-1B-keyword-research as one in-tandem research pass. The Topic Planner pulls it
...
metadata:
  ...
  version_history: >
    ...
    -> pod-1C-virality-research; full structural refactor to canonical CE skill
...
Virality Research is **Step 1C** of **Phase 1 (Research)** of the podcast pipeline, and it is OPTIONAL. The Research phase is LOCKED: `pod-1A-entity-research`, `pod-1B-keyword-research`, and `pod-1C-virality-research` run as ONE in-tandem research pass (Topic Only + Topic+Location scope). This skill rescores the candidate questions 2A and 2B surface; it never invents new topics.
...
Hi, I'm Virality Research. I'm an OPTIONAL step - if you skip me, the Topic Planner still works; if I run, the Topic Planner pulls in my scores. Before I run, I need to confirm the podcast architecture. If a `podcast-overview.md` exists for this client, I'll read it and confirm in one line. If not, I'll ask:
...
- **`entity-map.json`** - from `pod-1A-entity-research`. Cluster questions and bridge entities seed additional candidates.
- **`keyword-research.json`** - from `pod-1B-keyword-research`. PAA stacks and seed keywords seed additional candidates.
...
- **`podcast-overview.md`** - optional architecture doc, resolved via [Podcast Drive](https://docs.google.com/document/d/1YhybGpp9DIqmV56P6OOHIQe7A6RxvjQGHLHKcUM0JmU). If present, the skill auto-fills Greeting questions 1-3; otherwise it asks.
...
- **Upstream (optional, any of these):** `pod-1A-entity-research`, `pod-1B-keyword-research`, or an n-gram table. The skill runs on LLM-seed questions alone if all upstream is missing. The three Research-pass skills (2A / 2B / 2C) run in tandem.
...
Downstream consumers can rely on: the Google Doc URL is stable for the lifetime of the file (preserved via `files.update` across re-runs); `virality-research.json` validates against `references/schema/virality-research.json`; signal-score math reconciles. Upstream pulls (all optional): `entity-map.json` from `pod-1A-entity-research`, `keyword-research.json` from `pod-1B-keyword-research`, an n-gram table. If all upstream is missing, the skill runs on LLM-seed questions and flags `source_priority: "llm_seed_only"`.
...
- Resolve `n-gram-table.json` (Drive or local), `entity-map.json` (from `pod-1A-entity-research`), and `keyword-research.json` (from `pod-1B-keyword-research`) against the declared Inputs paths and JSON shapes.
...
- **Schema:** `~/.claude/skills/pod-1C-virality-research/references/schema/virality-research.json`.
...
- **Report back:**
  ...
  Next: pod-1A-entity-research and pod-1B-keyword-research run in tandem with this skill. Downstream: Topic Planner folds these scores into the ranking (downstream numbering pending).
  ```
