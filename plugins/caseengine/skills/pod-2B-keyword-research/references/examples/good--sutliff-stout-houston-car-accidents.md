...
1. Review this output with Gabe - if GOOD, save to `references/examples/good--car-accidents-houston.md` in the pod-2B-keyword-research skill folder.
2. Run `/pod-2A-entity-research` for Car Accidents (scope: Topic). Will consume seeds + PAA above.
3. Optionally run `/pod-2C-virality-research` for Car Accidents. Will mine PAA stacks.
