...
description: >
  ...
  four columns (Question, N-grams, Entities, Predicates), 20 rows PER LOCATION at
  every scope (Topic Only / Location / Extension). Use whenever someone says "n-gram table
...
metadata:
  ...
  version_history: >
    ...
    model (2026-07-10, Gabe directive from the Whalen scoping). 3.2.0 - render
    route aligned to the Entity Research formatting standard: shared branded
    .docx renderer (ngram-branded-docx.py), CE canonical cover, fixed column
    widths, Doc naming convention; all historical Docs re-rendered in place
    (2026-08-19). 3.3.0 - flat 20 questions per topic at every scope; the
    scope-dependent row counts (25-35 / 10-12) retired per Gabe directive;
    geo layering redesign deferred (2026-08-19).
---
...
An LLM Collation Table (sometimes called an N-Gram Table) for a single podcast episode. Four columns, 20 rows per location: Question Text, N-grams to Mention, Entities to Mention, Predicates to Mention. It is the episode's content backbone - every question becomes a beat in the Run of Show, every entity feeds the localized script, every predicate seeds the attorney response bullets. One table per episode per scope (Topic Only / Location / Extension), saved to Google Drive as the shared source of truth and consumed downstream by pod-3A-ros-template, pod-3B-client-ros, and pod-3C-client-guide.
...
3. **Extension locations (if any).** Extensions are sub-scope episodes - derivatives that inherit from the anchor but surface what's different at the smaller scope. Their table carries the same flat 20 questions as every other scope (Gabe directive 2026-08-19; the old ~10-question short format is retired). Can exist at any anchor level:
...
5. **Episode format** - resolved by the client's **Targeting strategy** (field 1 of the geo model; recorded in the podcast overview; ask if absent):
   ...
   - **Multi-location client** -> NO anchor/primary episode. Every topic produces one Mini episode (client-facing term; internal scope label stays `Extension`) per target city: 20 questions (the flat count every scope carries; the old 10-12 hard cap is retired, Gabe directive 2026-08-19). No hybrid (no full-plus-minis) unless the client brief explicitly says so.
   - Legacy anchor+extension model (anchor + per-city extensions, both at the flat 20 questions): still valid for single-location clients that add satellite markets; extensions inherit from the parent anchor and overlap-with-parent is by design.
...
- **Google Doc** - `N-Gram Table` - human-facing canonical view at the Drive destination below. Created / updated in-place via `files.update` against a stable fileId so the URL never breaks for downstream links. Typeface: Roboto for every text element (body, headings, table cells, captions), set by the shared renderer. Override only when a per-client `brand.json` typography block specifies otherwise. **Render route:** a plain markdown-to-GDoc conversion strips the cover and branding - build a branded `.docx` with the shared renderer `~/Desktop/claude_code/scripts/ngram-branded-docx.py` (thin sibling of `entity-map-branded-docx.py`, same cover / style helpers; implements the locked Doc order below, strips the pipe-delimited metadata line, truncates QA / provenance content) and upload it with `mimeType=application/vnd.google-apps.document` via `files.update` against the existing fileId (create once on first run, then always update in place). Same route as pod-1A Entity Research.
- **Cover page (required)** - the Google Doc opens with the CE canonical cover, then a page break before the report body. Layout matches the Entity Research cover: centered CE logo (`~/Desktop/claude_code/reference/brand/350x90 - dark.png`, ~3.0 in wide), doc-type title `N-Gram Table` bold 32pt in CE blue `#3573FF`, subtitle `{Episode Title} ({Location})` 14pt in `#0F172A` (Topic Only scope drops the location parenthetical), italic "Prepared by Case Engine" line, and a version/date line. Footer on every page: centered "Case Engine  |  Confidential" 8pt. Implemented in the shared renderer.
- **Table formatting (Doc render)** - fixed column widths, not auto-fit: the Question and N-grams columns take the most width; any `#` / row-number column gets a narrow fixed width (~0.3-0.4 in); compact font (8-8.5pt) and tight padding so the collation table targets roughly 2 pages. CE-blue header row with white bold header text. Implemented in the shared renderer (`collation_widths()` / `add_collation_table()`).
- **Doc naming (required)** - the Google Doc's `name` follows `{Topic} - {Location} N-Gram Table - {Episode Title}` (e.g., `Pedestrian Accidents - FL - Stuart N-Gram Table - Fault When a Pedestrian Is Hit`). At Topic Only scope drop the location segment: `{Topic} N-Gram Table - {Episode Title}` (in legacy topic-level folders with no episode layer, drop the episode segment too). Fill what exists, omit the rest - never a bare generic `N-Gram Table` name, which is unreadable in any list/search view. Applies to the Google Doc name only; `N-Gram Table.md` / `n-gram-table.json` keep their fixed filenames (downstream skills read them by name).
...
This destination is enforced by the **Canonical destination gate** in `### Quality gates

**Voice gate (hard, runs before publish).** Grep the Question Text column and FAIL on any hit; regenerate the offending row. Enforces Guideline 2.

- Abstract subjects: `\b(someone|anyone|people)\b` as the question's subject.
- Intensifiers: `\b(actually|really|truly)\b` - allowed ONLY as curiosity addressed to the attorney, which must be justified in the run notes.
- Personified vanishing: `disappear`, `vanish` applied to evidence or witnesses.
- Region pairing: `and across`, `or across` in question text.
- Covert listicle openers: `^(What|Which) (documents|records|evidence|witnesses|things|steps|factors)\b` without a stake or motive reframe.
- Trailing purpose clause: `\bto (protect|preserve|ensure|maximize)\b` at the end of a question.
- City-lane audit: every city token traces to the substance lane or the ranking lane (Guideline 4). Ranking-lane count <= 3 per location set.
- Named-entity fork count <= 1 per episode table (Guideline 2A).
- Carryover check: in a multi-location set, every city-free question is byte-identical across locations.
- Read-aloud pass: read every question out loud. A line no person would say FAILS regardless of whether it tripped a pattern above.
` (hard, pre-write). The four artifacts NEVER live in a client episode delivery folder - the dedicated `N-Gram Tables/` research library is the only valid write target.
...
1. **Targeting strategy** - `single-location` vs `multi-location`. Does the firm serve/rank one city or several? Drives episode format (Greeting Q5): single-location -> Full episodes; multi-location -> one Mini episode per target city (internal scope label stays `Extension`), no single primary episode. Either way the table carries the flat 20 questions.
...
- Row count: **20 rows PER LOCATION** (Gabe directive 2026-08-21). Topic Only = 20. A single-location Location/Extension table = 20. A multi-location set = locations x 20 (three cities = 60 rows). Downstream, `pod-3A-ros-template-v2` renders 15 of each location's 20 in the body and routes the remaining 5 to the swap pool WITH answer bullets built. This retires the previous flat-20-total contract. Do not pad to reach 20 and do not run long; 20 strong questions is the contract.
...
**Guideline 2 - Questions are VOICED, not composed.**

The governing test: **would a real person say this sentence out loud?** Not "is this a good search string" - a perfect search string is usually a terrible spoken question. The failure mode this guideline exists to kill is a question written from OUTSIDE the situation, describing a topic, when it should be written from INSIDE it - a person in the moment, or a host asking a practitioner about their experience. Origin: Eberst E2/E3/E4 (2026-08-21), where 22 of 30 generated questions needed a human rewrite before air.

- **Person - no abstract placeholders.** Banned as question subjects: `someone`, `anyone`, `people`, and generic-you-meaning-one. Use `I / my` when it is the caller's crisis moment ("I was just in a car accident in Stuart, what do I do?"), `you / your` for the listener's situation ("If you're badly hurt in a Stuart crash, where do they take you?"), or `you` addressed to the attorney's own experience ("What kinds of crashes do you see most in Stuart?"). BAD: "Where does someone badly hurt in a Stuart crash get treated?" / "How does anyone put a number on pain and suffering?"
- **Scenario first, question second.** Where a situation is implied, state it, then ask. "If [concrete situation], [question]?" BAD: "How long do you have to see a doctor after a crash in Florida?" GOOD: "If I don't see a doctor right after an accident in Florida, will that affect my PIP benefits?"
- **Concrete instance beats category.** The single most repeated fix in the source data. BAD: "commercial vehicle" / "Does Florida still pay you" / "What happens if the other driver has no insurance". GOOD: "box truck or delivery van" / "will your insurance still cover it" / "Who pays if the other driver doesn't have insurance?" Name the actual actor and the actual verb. Concrete instance nouns are ALLOWED in question text even when they are not entity-map entities - they are vividness, not authority, and Guideline 3 governs the Entities column only.
- **Plain searchable interrogative.** Match how a person actually asks: `how much`, `who pays`, `how long`, `what do I do`, `why does`. BAD: "What is a car accident claim actually worth in Stuart?" GOOD: "How much is a car accident claim worth in Stuart?"
- **No covert listicles.** The old ban caught only explicit "list 5 things". The real leak is enumeration disguised as a question: "Which witnesses matter most?", "What documents do I need?", "What medical records prove an injury?" - all produce mechanical listed answers. **Reframe enumeration into stake or motive.** BAD: "Which witnesses matter most, and how fast do they disappear?" GOOD: "Why do you have to get a witness's name and number right at the scene?" BAD: "What medical records prove an injury after a crash?" GOOD: "How much of your case comes down to your medical records?"
- **No writerly hooks, no loaded premises.** A question that pre-answers itself boxes the attorney in. BAD: "Why does settling early cost the most?" (assumes the answer) GOOD: "What do you give up by settling early?" A premise that merely reflects universal experience is fine - "Why is the first offer so low?" is allowed, because the answer is the strategy behind it, not the premise.
- **No personified vanishing.** Evidence and witnesses do not disappear on their own. BAD: "What evidence disappears first after a crash?" / "how fast do they disappear". GOOD: "What evidence do you lose by waiting?" - the listener's own delay, their own loss.
- **No trailing purpose clauses.** Fold the purpose into the question. BAD: "What should you do at the scene of a crash to protect the report?" GOOD: "How do you make sure the crash report gets your side right?"
- **Ban `actually` / `really` / `truly`.** Five of six occurrences in the source data were cut, one survived. The lone exception: genuine "how is that even possible" curiosity addressed to the attorney - "How do you actually put a number on pain and suffering?" Default is cut.
- **Singular only when forcing a pick is the point.** "What's the ONE piece of evidence that wins the most cases?" is a good on-air forcing question. "What is the most common mistake people make after a crash?" is wrong - there are obviously several, so it must be plural. Match grammatical number to reality.
- **Compound questions are fine when the second part is a real beat; cut when it is a hedge.** KEEP: "How do I get video of my crash in Stuart, and how long before it's gone?" CUT the hedge: "What happens to your case if your back ~~or your knee~~ was already bad before the crash?"
- **Repair ambiguity at generation, not downstream.** BAD: "How does where you got hit in Stuart change the fight over fault?" (body part or location?) GOOD: "How does where the crash happened in Stuart change the fight over fault?"
- **Length is NOT the lever.** Several correct rewrites are LONGER than the questions they replaced. Do not shorten to comply.
- **Why:** the table feeds a podcast Run of Show, not a blog post. Under the legacy chain `pod-3A-ros-template` lifts this column verbatim into a human's mouth, and under v2 it seeds the Short-Form block - there is no downstream stage that fixes voice. Whatever is written here is what gets said on air.
- **Where it fires in the SOP:** `## Create -> ### Generate questions`, and the voice gate in `### Quality gates`.

**Guideline 2A - Named-entity forks: allowed, encouraged, capped.**

- **The device:** at Location/Extension scope, put two real local agencies into the question text as an either/or the attorney resolves. GOOD: "If you crash in Stuart, who writes the report, Stuart PD or the Martin County Sheriff's Office?" / "Who responds to a car accident in Gainesville, the Alachua County Sheriff's Office or University of Florida Police?"
- **Cap: at most ONE per episode table, and never the same device in consecutive episodes of the same show.** Used every episode it stops being authority and becomes a tic.
- **Both agencies must come from the matching-scope entity map.** This is the one sanctioned case of entity names appearing in the Question Text column; Guideline 3 still governs the Entities column.
- **Why:** it is the strongest available local-authority signal in a spoken question, and it hands the attorney an immediate concrete fork instead of an abstract prompt.
...
- **Where it fires in the SOP:** `## Create -> ### Resolve the local anchor set (Location / Extension scope only)

What is?
The pass that gathers the city's real-world local stack BEFORE any question is written, so localization is a material the generator has on hand rather than something bolted on afterward. Skipped entirely at Topic Only scope.

- **Pull the anchor set from the matching-scope entity map.** Minimum viable set per city: the **city police department**, the **county sheriff**, the **CHP area office** with jurisdiction, the **county Superior Court**, the **county trauma center**, and **2-3 named freeways or corridors** that actually carry the traffic.
- **A thin map is a BLOCKER, not a shrug.** If the entity map holds only a city PD and a county court, STOP and backfill the map first. Generating against a thin map is what produces a table where the only local weight sits in the 3 city-tagged questions and the other 12 rows are indistinguishable from any other city's. Note the backfill in `metadata.local_anchor_backfill`.
- **Emit the resolved set to `local_anchors` in `n-gram-table.json`.** The Voice gate's local-signal floor reads this list. Without it the gate falls back to matching the city name alone, which under-counts badly - `UCI Medical Center` is Irvine signal and contains no "Irvine".
- **Where it fires:** before `### Generate questions`, and read again by `### Map n-grams, entities, predicates`.

### Localize: city in the question is a ceiling, city in the answer is a floor

What is?
The pass that decides WHERE each row's local weight lives. This is the single most-repeated failure in this skill's history, in both directions - questions stamped with the city until they read as spam, or a per-city table with no local weight at all.

- **Generate PER LOCATION.** Each city's table is generated against THAT city's entity map and anchor set. Never write one shared question core and copy it into every location folder. Cross-city verbatim overlap must stay **under 60%** (Voice gate enforces).
- **The city belongs ONLY in questions that are inherently local.** Who responds, where they take you, which courthouse, which freeway, how you get the report, where the IME happens. **Never inject the city into questions about legal doctrine, insurance mechanics, or timing** - comparative fault, UM/UIM, recorded statements, releases, and deadlines are state-level answers and a city token on them is the spam failure. The way to raise city density is to ADD a genuinely local question, never to bolt the city onto a universal one.
- **Target roughly a THIRD of questions carrying the city, and stop where it stops sounding natural.** Measured across the three Eberst episodes after editorial: 40% / 40% / 20%. **If you cannot reach the target without forcing, you are done - the number is a ceiling, not a quota** (Gabe directive 2026-08-21, after a run that hit 40% by bolting the city onto three universal questions and had to be reverted). Read every line aloud.
- **Every geo-neutral row still carries local weight in its ENTITIES and its answer bullets.** This is where the other two-thirds of localization lives: the question is universal, the answer names the local trauma center, the county court, the corridor, the responding agency. A row with no local signal in question, entities, OR n-grams is a MISS. The Voice gate enforces a **50% local-signal floor** across body rows.
- **Reword shared legal ground per city** rather than repeating it verbatim - the same scene question becomes "if you get hit on the 405" in Irvine and "on the 91" in Riverside. That is what keeps overlap under the cap while keeping the questions natural.
- **Where it fires:** `### Generate questions` and `### Map n-grams, entities, predicates`; enforced by the local-signal floor, cross-city overlap, and ranking-lane checks in `### Quality gates`.
...
- **PRIMARY TEST - the city must earn its place through one of exactly two lanes.** Every city token in a question must be justifiable as one of these. If it is neither, DELETE it - the question is stronger without it.
  - **Substance lane** - the ANSWER is genuinely different because of the city: which agency responds, which trauma center, which courthouse, which local roads or camera systems. GOOD: "If you crash in Stuart, who writes the report, Stuart PD or the Martin County Sheriff's Office?" / "How do I get video of my crash in Stuart, and how long before it's gone?"
  - **Ranking lane** - a deliberate money-phrase retrieval target where the answer is general but the city rides for search. Cap at 2-3 per location set. GOOD: "How much is a car accident claim worth in Stuart?" / "How long do you have to file a car accident lawsuit in Stuart?"
  - **Neither lane = cut the city.** BAD: "What should you do at the scene of a crash in Stuart to protect the report?" (identical answer in any city, not a money phrase) -> "How do you make sure the crash report gets your side right?" BAD: "What medical records prove an injury after a crash in Stuart?" -> "How much of your case comes down to your medical records?"
- **REGION PAIRING IS BANNED FROM SPOKEN QUESTION TEXT.** Constructions like "in Stuart and across the Treasure Coast" / "in Gainesville and across North Central Florida" were deleted by editorial in 6 of 6 occurrences. The regional retrieval intent is real and is preserved in the N-grams column and the downstream answer bullets - NOT in the question a human reads aloud. (Interacts with `pod-3A-ros-template-v2` Guideline 6; that skill owns the generation-side pairing rule.)
- **LOCALIZATION LIVES IN THE ANSWER, NOT THE QUESTION (Gabe directive 2026-08-21).** The city is a **ceiling in the question text and a floor in the answer**. A geo-neutral question is normal and correct - but its Entities column and its downstream answer bullets must still carry the target city's local stack, so the attorney names the local freeway, the local trauma center, the county court, the local agency while answering it. A row whose question, entities, AND n-grams all carry zero local signal is a MISS, not a neutral row. The point of shipping a per-city episode is that it sounds like that city; if only the 3 city-tagged questions carry local weight, the block is 80% generic and the per-city split earns nothing.
- **Tables are generated PER LOCATION, never copied.** Each city's table is generated against THAT city's entity map with city-specific signals leading (see `## Create -> ### Generate questions`, multi-location Mini rule). **The cities' tables must not overlap verbatim** - partially different question mixes, shared legal ground reworded per city. A shared question core copied byte-identical into every location folder is a defect: it is what produces a set of blocks that are ~80% the same and it violates the multi-location Mini rule that predates this guideline. (A byte-identity carryover rule was briefly added here on 2026-08-21 and RETRACTED the same day - it contradicted the pre-existing rule and flattened local density to 20%.)
- **Secondary sanity check (not the rule):** at Location/Extension scope the table must READ as a local show - the city is present and grounding - WITHOUT being stamped onto every line. Aim for the city to land naturally in roughly a THIRD of the questions (a soft center of gravity, not a quota): enough that a listener immediately knows this is a [City] show, never so much that it reads as keyword spam. The city belongs where it falls naturally - the local market, a named local actor, a venue, "here in Glen Burnie", "which Columbia courthouse handles...". The remaining ~two-thirds carry locality through named local entities (the corridor, the county court, the trauma center, the local agency) or stay geo-neutral - both are fine. HARD CEILING: the city in more than HALF the questions reads as spam and FAILS. GUARD THE OTHER WAY TOO: a table where the city is essentially absent is too thin - it must still read as local. Any question whose substance is state law NAMES the state ("Florida's 14-day PIP rule") and does not get a city token bolted on - that is not a city mention. The county appears only where it earns its place (the court sits there, a county agency is the actual actor). Locality is reinforced by the localized ENTITY stack (Guideline 1) running through every row, not by the city token alone - so the non-city questions still signal local authority.
...
- **Row count** - 20 rows per location (multi-location = locations x 20), per Quality bar.
...
- **Topic Only example tables run short.** The bundled Topic Only worked example runs 18 rows; the target is a flat 20. The short example is illustrative, not a row-count anchor.
...
- Generate exactly 20 questions PER LOCATION (see `### Quality bar`). Three cities = 60 questions total. Of each location's 20, the strongest 15 become the ROS body and the remaining 5 become that location's swap pool - but ALL 20 get full n-grams, entities, and predicates here, so a swap downstream never requires a rebuild.
- **Carryover discipline (multi-location).** The city-free questions are the shared core and must be byte-identical across every location set (Guideline 4). Only lane-justified city questions vary per location. Aim for roughly 3 city-bearing and 17 shared per 20; the exact split follows the two-lane test, never a quota.
...
- **Render rules - the client-facing Google Doc.** These are structural, not suggestions. Every n-gram Doc renders identically clean and sharp with zero manual cleanup. **Render route:** the shared renderer `~/Desktop/claude_code/scripts/ngram-branded-docx.py` builds a branded `.docx` from the markdown source (it implements every rule below - locked section order, cover, static text blocks, table styling, metadata-line strip, QA truncation, Roboto throughout); upload the `.docx` with `mimeType=application/vnd.google-apps.document` via `files.update` against the existing fileId, setting the Doc `name` per the Doc naming convention (`### Outputs -> #### Output formats`) in the same call. Never `files.create` against an existing table.
  - **Doc body content.** Truncate the markdown source at the first `## Quality Assurance` heading (exclude QA / provenance / handoff sections wherever they appear). Strip the internal metadata header line (the pipe-delimited `Topic: ... | Industry: ...` provenance line) from the body. The Doc renders in the locked order: Cover -> Executive Summary (heading -> generated narrative paragraph -> stat bullets) -> Methodology (hard-coded) -> Collation table (~2 pages, blue header) -> table explainer (hard-coded) -> INTERNAL section.
  - **Branded cover page.** CE canonical cover per the Entity Research standard (implemented in the shared renderer): centered CE logo (`~/Desktop/claude_code/reference/brand/350x90 - dark.png`, ~3.0 in), title `N-Gram Table` bold 32pt in CE blue `#3573FF`, subtitle `{Episode Title} ({Location})` 14pt (e.g., `How to File a Car Accident Claim (Houston, TX)`; Topic Only drops the parenthetical), italic "Prepared by Case Engine", version/date line, then a page break.
  ...
  - **Collation table styling.** The 4-column table (Question Text | N-grams | Entities | Predicates) renders with a CE brand-blue (`#3573FF`) background on the header row and white bold header text. Fixed column widths, not auto-fit (Question and N-grams widest; a `#` / row-number column, if present, gets a narrow ~0.3-0.4 in fixed width). The table targets roughly 2 pages: compact font size (8-8.5pt), tight row height, trimmed cell padding - readable but tight. Bold the strongest 1-2 entities per row in the Entities column; when scope is Location/Extension, the localized instance is always one of the bolded entities.
  ...
  - **Roboto.** The shared renderer sets Roboto on every text element (body, headings, table cells, captions) inside the `.docx`, so the font survives the Drive conversion - no separate `batchUpdate` pass is needed. Override only when a per-client `brand.json` typography block specifies otherwise.
  - **Footer.** Centered `Case Engine  |  Confidential` 8pt on every page, set by the shared renderer.
...
Content Requirements:
 - Questions must sound natural and conversational, suitable for a legal podcast discussion.
   NOTE (2026-08-21): this legacy line is INSUFFICIENT on its own and is retained only as historical
   provenance. Question voice is governed by `### Editorial Guidelines -> Guideline 2 (Questions are
   VOICED, not composed)` and the BAD->GOOD calibration pairs in
   `references/examples/n-gram-table-examples.md`. Where this prompt and Guideline 2 disagree,
   Guideline 2 wins.
...
(Full 18-row reference table in `references/examples/n-gram-table-examples.md`. Note: 18 rows is the illustrative length - the target is a flat 20.)
...
Observations: the Topic Only example runs 18 rows but the target is a flat 20. The City-level example runs 21 rows with 3-5 entities each, denser than Topic Only by design. The State-level example threads 2 major cities - that is the pattern.
...
- [ ] Watch for tables shipping off the flat 20-row count (short or long, any scope); if it recurs, tighten the `### Generate questions` guidance.
...
| 2026-08-19 | **v3.2.0 - render route aligned to the Entity Research formatting standard (S1668 alignment).** The client-facing Google Doc now ships via the shared branded-`.docx` route: `~/Desktop/claude_code/scripts/ngram-branded-docx.py` (thin sibling of `entity-map-branded-docx.py`, reusing its cover / style helpers) builds the `.docx` and it uploads with `mimeType=application/vnd.google-apps.document` via `files.update` against the existing fileId - same route as pod-1A Entity Research. Cover aligned to the CE canonical cover (centered CE logo `350x90 - dark.png` ~3.0 in, title `N-Gram Table` bold 32pt CE blue #3573FF - was 36pt - subtitle 14pt, italic "Prepared by Case Engine", version/date line, page break; footer "Case Engine  \|  Confidential" 8pt centered). Collation table gets fixed column widths (not auto-fit; Question/N-grams widest, narrow `#` column) plus the existing CE-blue header row + white bold header text, compact ~2 pages. New **Doc naming (required)** convention: `{Topic} - {Location} N-Gram Table - {Episode Title}`; Topic Only drops the location segment; never a bare generic `N-Gram Table` name. The separate post-upload Roboto `batchUpdate` pass is retired - the renderer sets Roboto inside the `.docx`. All 469 historical N-Gram Table Docs in the `N-Gram Tables/` library re-rendered and renamed in place same day (fileIds/URLs preserved; batch tooling in `~/Desktop/claude_code/scripts/drive-ngram-resync/` - `inventory.py` + `rerender.py`). Revert: restore the 3.1.0 Output formats Google Doc bullet + Ship render-rules bullets (36pt title, Branding-folder resolve, Roboto batchUpdate pass, `Page {PAGE}` footer), drop the Cover page / Table formatting / Doc naming bullets. |
...
| 2026-08-21 | **v3.4.0 - question voice overhaul + 20-per-location row count (Gabe directive 2026-08-21).** Guideline 2 rewritten from a one-line anti-listicle rule into a full voicing spec ("VOICED, not composed"): person rules (no `someone`/`anyone`/`people`; I/my for crisis, you/your for listener, you for attorney experience), scenario-first structure, concrete-instance-over-category, plain searchable interrogative, covert-listicle reframe into stake/motive, no writerly hooks or loaded premises, no personified vanishing, no trailing purpose clauses, `actually`/`really`/`truly` banned with one narrow exemption, grammatical number matched to reality, hedge-vs-real-beat distinction for compounds, ambiguity repaired at generation. Explicit note that length is NOT the lever. New Guideline 2A (named-entity forks, capped at one per episode table). Guideline 4 restructured around a two-lane city test (substance lane / ranking lane; neither = cut the city) with the percentage demoted to a sanity check, region pairing BANNED from spoken question text, and a multi-location carryover rule (city-free questions byte-identical across locations). New Voice gate in Quality gates with greppable patterns plus a read-aloud pass. Row count changed from flat-20-total to **20 per location** (three cities = 60 rows); downstream 3A-v2 renders 15 per location in the body and routes 5 to a swap pool WITH bullets. `references/examples/n-gram-table-examples.md` rebuilt with 30 real BAD->GOOD pairs from Eberst E2/E3/E4 and the 8-question shipped-unedited pass set; the pre-existing worked tables flagged as superseded for Question Text and still valid for the other three columns. Origin: Eberst editorial pass where 22 of 30 generated questions required human rewrite. Revert: restore SKILL.md and the examples file from the 2026-08-21 scratchpad backups. | Gabe Jordan |
| 2026-08-19 | **v3.3.0 - flat 20 questions per topic, every scope (Gabe directive 2026-08-19).** The scope-dependent row counts are retired: Topic Only 25-35, Location 25-35, and Extension/Mini 10-12 all become exactly 20 rows, regardless of scope. Touched: frontmatter description, `### Quality bar`, `### Quality gates` row-count gate, `## Create -> ### Generate questions` (including both Extension branches - selection/composition logic unchanged, output count now 20), Greeting Q3/Q5, `### Framing` field 1, the Gotchas short-example note, the INTERNAL reference prompt ("Include 20 rows"), worked-example observations, and Learning & Iteration. How the location-specific layering gets baked in on top of the flat 20 is explicitly DEFERRED - Guideline 1, Guideline 4, the localization scan, and the city-share check are untouched. Revert: restore the 25-35 / 10-12 scope-dependent counts across the same sections and set version back to 3.2.0. |
