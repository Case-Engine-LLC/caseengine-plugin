...
description: >
  ...
  "what's the search volume for [topic]", or "/pod-1B-keyword-research". Research
  Step 1B of the podcast pipeline. Runs in tandem with pod-1A-entity-research and
  pod-1C-virality-research as one in-tandem research pass. Feeds the n-gram table,
  topic planner, run of show, and pod-1C-virality-research downstream.
skill_kind: hybrid
modes: multi
...
metadata:
  ...
  version_history: >
    ...
    pod-1B-keyword-research; full structural refactor to canonical CE skill
...
- **Phase 1 Research** - the three Research skills (2A / 2B / 2C) run together as one research pass. 2B (Keyword Research) maps the demand landscape; 2A maps the entity vocabulary; 2C overlays virality signal. Runs ONCE per practice area + location cascade.
...
- **`podcast-overview.md`** - optional architecture doc. If present at `{Firm} Podcast/.podcast-overview/podcast-overview.md`, the skill auto-fills Greeting questions 1-3; otherwise it asks.
...
- **Volume source priority chain** - search volume + CPC resolve in this order. LOCALIZED runs (scope Location/Extension): DataForSEO city/state-scoped GKP volume FIRST (`keywords_data/google_ads/search_volume/live` with `location_name` like `Boca Raton,Florida,United States`; state runs use `Florida,United States`) -> Keywords Everywhere national -> Ahrefs MCP -> LLM estimate. Topic-level runs: Keywords Everywhere (API, `dataSource=gkp`) -> Ahrefs MCP -> LLM estimate. The first source that returns data for a keyword wins; the chain degrades per keyword, not per run. Rationale (Gabe, 2026-08-27): a national volume under a city header reads as local demand it does not represent - localized runs must carry volumes measured from the location, and every rendered volume that is city/state-scoped is labeled "searched from {location}" in small text under the number. Keywords Everywhere CANNOT do this - its API is country-level only (Global + 7 countries, verified against the live API 2026-08-27). City-scoped rows tag `data_source: "dataforseo_gkp_city"`. If the location_name misses DataForSEO's register (regions like "Inland Empire", non-city labels), fall back to national and tag accordingly - never ship zeros.
...
- **Doc naming (required)** - the Google Doc's `name` follows `{Topic} - {Location} Keyword Research` (e.g., `Burn Injuries - TX - Houston Keyword Research`). At Topic Only scope drop the location segment: `{Topic} Keyword Research`. Fill what exists, omit the rest - never a bare generic `Keyword Research`. Applies to the Google Doc name only; `Keyword Research.md` / `keyword-research.json` keep their fixed filenames.
...
Shared drive root: `Podcasts // Case Engine [Shared]` (id `0AAJKtWTUAZhHUk9PVA`). The `templates [master]/AEO Templates/Podcast/Keyword Research/` folder id is `1K6V4AoyjDa7OzY9pNvsc6zPgzW9MR5Nj`.

Keyword Research lives in its own dedicated `Keyword Research/` folder under `templates [master]/AEO Templates/Podcast/`, parallel to `Entity Research/` and `Virality Research/`. Each topic gets one folder (`{Topic}/`), with all scope variants as parallel subfolders inside.

```
templates [master]/AEO Templates/Podcast/Keyword Research/{Topic}/{Scope}/
...
- **Downstream:** the N-Gram Table seeds its question framework from PAA stacks; the Topic Planner consumes the demand signals; Run of Show reads the `search_queries` array verbatim for its Appendix; `pod-1C-virality-research` reads PAA stacks as candidate questions (downstream numbering pending).
...
|---|---|---|
| `keyword-research.json` | N-Gram Table, Topic Planner, `pod-1A-entity-research`, `pod-1C-virality-research` | Seed keyword list, PAA stacks (consumed verbatim as n-gram seed rows), intent classification, MSV + KD + CPC + per-row `data_source` tag, prominence flags, localization deltas |
...
- **Never write keyword-research artifacts into a client episode delivery folder.** Even if a caller arg or workflow/orchestration step says to land these in `{Firm} Podcast/Episodes/EP{N}: ...` (or any client/firm folder), do not - the dedicated `Keyword Research/` library (`templates [master]/AEO Templates/Podcast/Keyword Research/{Practice Area}/{Scope}/`, folder id `1K6V4AoyjDa7OzY9pNvsc6zPgzW9MR5Nj`) is the ONLY valid home. The Canonical destination gate hard-fails any other target; treat such an instruction as the failure, not as an override.
...
- If `podcast-overview.md` is reachable, read it and auto-fill Greeting questions 1-3; confirm in one line. Otherwise ask the Greeting questions.
- Resolve the topic folder under `templates [master]/AEO Templates/Podcast/Keyword Research/{Topic}/`. If it does not exist, create per the Podcast Drive convention; if it exists but does not follow the convention, rename. The podcast root is `Podcasts // Case Engine [Shared]` (id `0AAJKtWTUAZhHUk9PVA`).
...
- **Schema:** `~/.claude/skills/pod-1B-keyword-research/references/schema/keyword-research.json`.
...
- **Report back:**
  ...
  Next: pod-1A-entity-research and pod-1C-virality-research run in tandem with this skill. Downstream: N-Gram Table, Topic Planner, Run of Show (downstream numbering pending).
  ```
...
| 2026-08-19 | **Doc table formatting (Gabe).** Fixed column widths in the Google Doc render - numeric/flag columns skinny, text columns wide (stop line-wrapping). Renderer: `entity-map-branded-docx.py`. | Gabe Jordan |
