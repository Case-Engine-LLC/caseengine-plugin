...
- **The 2 STATIC strings resolve, they do not regenerate.** `welcome` (or `welcome_first` on Episode 1) equals the template constant with `{{PODCAST_NAME}}` / `{{ATTORNEY_NAME}}` resolved; `outro_note` is byte-identical. Embedded-name shows never render "with **{{ATTORNEY_NAME}}**", which doubles the name (Gabe 2026-08-26, locked): a podcast name of the shape "{Prefix} w./with {Attorney}" renders the SPLIT SPOKEN form - "Welcome back to the {Prefix} podcast with {Attorney}." in plain text with NO bolds, the attorney said once and "podcast" in the middle - while a name that embeds the attorney with no "w./with" to split at takes the `welcome_embedded` / `welcome_embedded_first` fallback constants. A "w." is always spoken as "with". The renderer compares against the constants and hard-fails on any other delta (Editorial Guideline 8 upstream).
...
- **STATIC resolved-verbatim gate.*** `welcome` / `welcome_first` equal the template constant with tokens resolved; embedded-name shows compare against the SPLIT SPOKEN form for "w./with" names, else the `welcome_embedded` fallback constants (Gabe 2026-08-26, locked); `outro_note` byte-identical. Any other delta means boilerplate was regenerated.
...
- Set `static.welcome` to the resolved `welcome_first` constant when this is Episode 1 of the show (`is_first_episode: true`) - otherwise the resolved `welcome`. Embedded-name shows: a "{Prefix} w./with {Attorney}" name takes the computed SPLIT SPOKEN form ("Welcome back to the {Prefix} podcast with {Attorney}.", plain, no bolds); a no-splice embedded name takes the resolved `welcome_embedded` / `welcome_embedded_first` fallback (Gabe 2026-08-26, locked).
...
| Date | Change |
|---|---|
| 2026-08-26 | **Line 1 split form locked (Gabe, 2026-08-26, consolidated re-sync from ce-ros-v2 assemble.ts).** A "{Prefix} w./with {Attorney}" podcast name renders the SPLIT SPOKEN welcome at populate - "Welcome back to the {Prefix} podcast with {Attorney}.", plain text, NO bolds, the attorney said once with "podcast" in the middle. The "the {{PODCAST_NAME}} Podcast." constants demoted to the fallback for embedded names with no "w./with" splice point. The welcome gate in `scripts/build-client-ros-v2-docx.py` accepts the standard constant, the split form, and the fallback (with "w." spoken as "with"). | Gabe Jordan |
