...
description: >
  ...
  "build an entity map", "koray entity map", or "/pod-1A-entity-research". Research
  Step 1A of the podcast pipeline - the jurisdiction-agnostic foundation every
  downstream skill reads. Runs in tandem with pod-1B-keyword-research and
  pod-1C-virality-research as one in-tandem research pass. Feeds the n-gram table,
...
metadata:
  ...
  version_history: >
    ...
    pod-1A-entity-research; full structural refactor to canonical CE skill
...
- **Phase 1 Research** - the three Research skills (2A / 2B / 2C) run together as one research pass. 2A (Entity Research) is the foundation the other two layer onto; 2B and 2C overlay demand and virality signal. Runs ONCE per practice area + location cascade.
- **Phase 2 Planning** - `pod-2A-topic-planner` ranks episodes from the research; `pod-2B-n-gram-table` builds the per-episode question framework.
- **Phase 3 Run of Show** - `pod-3A-ros-template`, `pod-3B-client-ros`, and `pod-3C-client-guide` run per prioritized episode.
...
If a `podcast-overview.md` doc is present for this client, I'll read it to auto-fill the architecture questions below (firm name, anchor scope, extensions). Otherwise I'll ask you directly.

#### Propose the plan

Once I have the architecture (auto-filled from `podcast-overview.md` or from your answers), I propose the full plan in one message and you confirm or modify:
...
#### Architecture questions (asked when podcast-overview.md is absent)

I'll ask for the three geo fields (canonical geo model - use these exact labels):
...
- **Practice Area** - the practice area or topic name. Resolves to the `{Practice Area}/` Drive folder under `templates [master]/AEO Templates/Podcast/Entity Research/`. Examples: Car Accidents, Wrongful Death, Premises Liability.
...
- **`keyword-research.json`** - the keyword-research seed set from `pod-1B-keyword-research` at `templates [master]/AEO Templates/Podcast/Keyword Research/{Practice Area}/{matching scope}/`. When present, PAA stacks seed candidate entity surface area. Optional signal overlay, not a required input.
...
- **`podcast-overview.md`** - optional architecture doc (firm name, anchor scope, extensions), resolved via [Podcast Drive](https://docs.google.com/document/d/1YhybGpp9DIqmV56P6OOHIQe7A6RxvjQGHLHKcUM0JmU). If present, auto-fills the Greeting questions; otherwise the Greeting asks.
...
- **Cover page (required)** - the Google Doc opens with the CE canonical cover, then a page break before the report body. Layout mirrors the Client ROS / Client Guide cover (`~/Desktop/claude_code/scripts/cowork-podcast-branded-docx.py`, `add_cover()` - the same cover pattern the retired Podcast Bible used): centered CE logo (`~/Desktop/claude_code/reference/brand/350x90 - dark.png`, ~3.0 in wide), doc-type title `Entity Research` bold 32pt in CE blue `#3573FF`, subtitle `{Practice Area} - {Scope label}` 14pt in `#0F172A`, italic "Prepared by Case Engine" line, and a version/date line. Typeface on the cover follows this Doc's Roboto rule (the reference script's Calibri is superseded here). Footer on every page: centered "Case Engine  |  Confidential" 8pt. **Render route:** a plain markdown-to-GDoc conversion strips the cover and branding - build a branded `.docx` (cover + body, python-docx per the reference script) and upload it with `mimeType=application/vnd.google-apps.document` via `files.update` against the existing fileId (create once on first run, then always update in place).
- **Table formatting (Doc render)** - fixed column widths, not auto-fit: numeric / flag / short-code columns (scores, counts, tiers, `*`/`-` flags) get a narrow fixed width (~0.4-0.6 in) so the text-bearing columns (entity / keyword / question, type, cluster) take the freed width and stop wrapping across many lines. Implemented in the shared renderer `~/Desktop/claude_code/scripts/entity-map-branded-docx.py` (`column_widths()`).
- **Doc naming (required)** - the Google Doc's `name` follows `{Practice Area} - {Location} Entity Research` (e.g., `Car Accidents - FL Entity Research`, `Lemon Law - CA - San Diego Entity Research`). At Topic Only scope drop the location segment: `{Practice Area} Entity Research`. Fill what exists, omit the rest - never a bare generic name. The client-facing display label is **Entity Research** (skill/methodology keeps "entity map" internally), which is unreadable in any list/search view. Applies to the Google Doc name only; `Entity Map.md` / `entity-map.json` keep their fixed filenames (downstream skills read them by name). **Vector strength displays to 2 decimal places in the Doc render** (e.g., `0.94`, not `0.935`); `entity-map.json` and the markdown keep full 3-decimal precision.
...
Shared drive root: `Podcasts // Case Engine [Shared]` (id `0AAJKtWTUAZhHUk9PVA`). The `templates [master]/AEO Templates/Podcast/Entity Research/` folder id is `1iKEslJ5JWScmRwjTAEUDrAvBakzQ81Ee`.

Entity Research lives in its own dedicated `Entity Research/` folder under `templates [master]/AEO Templates/Podcast/`, parallel to `Keyword Research/` and `Virality Research/`. Each practice area gets one folder (`{Practice Area}/`), with all scope variants as parallel subfolders inside.

```
templates [master]/AEO Templates/Podcast/Entity Research/{Practice Area}/{Scope}/
...
- **"What Is Entity Research?"** - FIXED intro block, first section after the cover, identical on every doc (hardcoded in the renderer `entity-map-branded-docx.py`, NOT AI-written per run). Canonical text, verbatim:
  - An entity is any distinct thing a topic is made of - the concepts, laws, parties, processes, and organizations that define it (e.g., "negligence," "statute of limitations," "insurance adjuster").
  - Search engines and AI answer engines decide whether content is authoritative by checking whether it covers the entities they associate with a topic - and connects them the way experts do.
  - This report gathers the entities for this topic, scores and weights them, and ranks which matter most - so every downstream effort (podcast episodes, website optimization, content) is built on the same prioritized vocabulary.
- Summary Findings (entity count, tier distribution, cluster + bridge counts, localization scan result) - heading is `Summary Findings` in the client-facing Doc; the renderer also maps any legacy `Executive Summary` heading to `Summary Findings`
...
- **Upstream (optional):** `pod-1B-keyword-research` - keyword research can run in parallel; this skill runs with just a topic name. The three Research skills run as one in-tandem pass.
- **Downstream:** `pod-2A-topic-planner`, `pod-2B-n-gram-table`, and the Phase 3 Run of Show skills, plus SEO topical maps, content briefs, website architecture all consume `entity-map.json` at matching scope.
...
|---|---|---|
| `entity-map.json` | `pod-2A-topic-planner`, `pod-2B-n-gram-table`, the Phase 3 Run of Show skills, `pod-1C-virality-research` | Scored entity list with tiers + vector strengths + cluster assignments + bridge flags + localization coverage + optional `localization_supplement` block + provenance |
...
Downstream consumers can rely on: the Google Doc URL is stable for the lifetime of the map (preserved via `files.update` across re-runs); `entity-map.json` validates against `references/schema/entity-map.json`. Upstream pulls (all optional): `keyword-research.json` from `pod-1B-keyword-research`, a parent `entity-map.json` at the `Topic Only/` path (inheritance source), a content-gap report (auto-detected locally or in `_inputs/`).
...
- **Where it fires in the SOP:** `## Checks -> ### Orient` (resolve the three geo fields from podcast-overview.md or the greeting) and `## Create -> ### Cluster and bridge` / `### Evaluate localization coverage` (build at the anchor breadth; localize toward the Episode geo target as a ceiling, never a floor).
...
- **Template folder only.** This skill writes under `templates [master]/AEO Templates/Podcast/Entity Research/`. Client populate is downstream.
...
- If `podcast-overview.md` is reachable, read it and auto-fill the Greeting questions; confirm in one line. Otherwise ask the Greeting questions.
- Verify the practice area folder under `templates [master]/AEO Templates/Podcast/Entity Research/{Practice Area}/`. If it exists and is named correctly, proceed. If it exists but does not follow the convention, rename per Podcast Drive. If it is missing, create per the Podcast Drive decision tree.
...
- **Check for keyword research.** Look for `keyword-research.json` at `templates [master]/AEO Templates/Podcast/Keyword Research/{Practice Area}/{matching scope}/`. If found, mine PAA stacks (candidate entity surface area), high-volume head terms (Tier 1 cross-check), cluster seeds. Log `keyword_research: found | not_found`.
...
- **Cover page.** Ship the Doc via the branded `.docx` route (cover + body uploaded as `application/vnd.google-apps.document` against the stable fileId) so the CE canonical cover survives - see `### Outputs -> #### Output formats -> Cover page`. A cover-less Doc fails QA.
...
- **Report back:**
  ...
  Next: pod-1B-keyword-research and pod-1C-virality-research run in tandem with this skill. Downstream: pod-2A-topic-planner, pod-2B-n-gram-table, then Phase 3 Run of Show.
  ```
...
| 2026-05-20 | **v3.0.0** - renamed `pod-2-entity-research` -> `pod-1A-entity-research` (folder + slug + frontmatter `name`). Full structural refactor to the canonical CE skill structure mirroring `pod-5-n-gram-table` v3.0.0. Killed Mode A / Mode B environment-name branching - replaced with `### Probe environment` capability probe (FS-read / FS-write / Drive / RAG tiers); local mirror writes when the FS-write probe succeeds. Frontmatter completed (`skill_kind`, `modes: multi`, `inputs`, `outputs`, `notify`; `version`/`date`/`owner` moved to a `metadata` block). Best Practices restructured to the canonical contract H3s (Inputs / Outputs / Framing / Quality bar / Sourcing discipline / Editorial Guidelines / Quality gates / Gotchas / Iteration log); vector / tier / cluster / bridge / inheritance / localization methodology relocated into Quality bar + Editorial Guidelines + the deliverable-shaped Create buckets. SOP rebuilt as H2 phase siblings (Checks / Prepare Inputs / Create / Update / Quality Assurance / Ship); `## Capture data` -> `## Prepare Inputs`, `## Push to Drive` -> `## Ship`, `## Backfill` folded into `## Update` + `## Ship`. QA rewritten as the canonical three-tier gate with the hardwired Anti-AI Detection two-pass scan and an On-failure recovery line. Workflow demoted to H3 with the locked Research-phase diagram (1A/1B/1C in tandem) + YOU ARE HERE; downstream stages drawn as named boxes without codes (downstream numbering pending). `references/schemas/` fixed to `references/schema/`. Owner Gabe Jordan. |
| 2026-07-31 | v3.1.0 - removed the pod-1-podcast-bible dependency (architecture now comes from the Greeting + optional podcast-overview.md), all Fortress/DB access, and the vestigial "no numbered lists in ClickUp" QA clause, ahead of the skill moving to an environment without CE infra. Historical iteration-log entries left intact. | Gabe Jordan |
| 2026-08-19 | **Cover page requirement added (Gabe).** Real-run review of a shipped Doc showed no cover page. Added required CE canonical cover (logo / 32pt CE-blue doc-type title / practice-area+scope subtitle / Prepared-by / version line, page break before body) to `#### Output formats`, mirroring the Client ROS cover in `cowork-podcast-branded-docx.py` (the old Podcast Bible cover pattern) with Roboto per this Doc's typeface rule. Ship path changed from plain MD-to-GDoc to branded `.docx` uploaded as `application/vnd.google-apps.document` via `files.update` (plain conversion strips branding). Cover-less Doc now fails QA. **Revert:** remove the Cover page bullet from Output formats and the Cover page bullet in How it ships; restore plain MD-to-GDoc render. | Gabe Jordan |
| 2026-08-19 | **Doc table formatting (Gabe).** Fixed column widths in the Google Doc render - numeric/flag columns skinny, text columns wide (stop line-wrapping); vector strength shown at 2 decimals in the Doc (JSON/markdown keep 3). Renderer: `entity-map-branded-docx.py`. | Gabe Jordan |
| 2026-08-19 | **Doc naming template (Gabe).** Google Doc name changed from the bare doc-type to `{Topic} - {Location} {Doc Type}` (location dropped at Topic Only) so docs are distinguishable in list views. Markdown/JSON filenames unchanged (read by name downstream). | Gabe Jordan |
| 2026-08-19 | **Display label change (Gabe).** Client-facing Google Doc label changed from "Entity Map" to "Entity Research" - cover title and Doc-name template both use Entity Research. Internal names unchanged (`Entity Map.md`, `entity-map.json`, skill methodology wording). | Gabe Jordan |
| 2026-08-19 | **Fixed intro block (Gabe).** Added the fixed intro block as the first client-facing section (heading renamed same-day to "What Is Entity Research?") after the cover - generalized, deliverable/location-agnostic, hardcoded verbatim in the renderer so it never varies per run (entity definition / why engines care / what this report does). | Gabe Jordan |
| 2026-08-19 | **Chart redesign + tighter numeric columns (Gabe).** Vector-space chart was unreadable embedded (12in figure squeezed to 6.5in; 49 overlapping labels). `entity-vector-space.py` redrawn at consumption size (6.5x5.0in @300dpi), labels only Tier 1 + bridges via deterministic rim callouts (even slots around the ring, leader lines to dots - no overlap by construction), names shortened (parentheticals + state prefix dropped, 32-char cap), legend below chart. Table numeric/flag columns tightened to a flat 0.38in (fits `1.00`). Both in the renderer + chart script. | Gabe Jordan |
| 2026-08-19 | **Real heading tags + Data source paragraph removed (Gabe).** Doc render now uses actual Word heading styles (md H2/H3/H4 -> Heading 1/2/3, intro block = Heading 1) so the Google Doc gets a working outline/navigation, and the renderer drops any paragraph starting `Data source:` from the client-facing render (provenance stays in the markdown / metadata.json). | Gabe Jordan |
| 2026-08-19 | **Vector-strength formula relocated (Gabe).** The formula code block moves out of the Executive Summary and renders at the end of the "What You're Looking At" intro ("How each entity is weighted:" + formula line). Renderer extracts it from the markdown automatically. | Gabe Jordan |
| 2026-08-19 | **Vector Space section blurb removed (Gabe).** The "Entities plotted radially..." methodology paragraph no longer renders in the client-facing Doc - the Vector Space Visualization section is heading + chart only. Markdown keeps the paragraph internally. | Gabe Jordan |
| 2026-08-19 | **Vector-space chart final design (Gabe).** `entity-vector-space.py` now renders: tier bands shaded at the true strength thresholds (T1 >=.80 / T2 .60-.79 / T3 .40-.59) with radius rescaled so vs .35-1.0 fills the circle (jitter is angular-only - dots never cross their band); ALL entities labeled via elliptical 360 callouts (fixed vertical slot spacing per side - overlap impossible; leader lines to dots); label font size scales with vector strength (~4.5-10pt); names shortened (parentheticals + state prefix dropped). 6.5x5in @450dpi. | Gabe Jordan |
| 2026-08-19 | **"Executive Summary" renamed to "Summary Findings" (Gabe).** Client-facing Doc heading is now Summary Findings; renderer maps legacy Executive Summary headings automatically. | Gabe Jordan |
| 2026-08-19 | **Chart moved into Summary Findings (Gabe).** The vector-space chart now renders inside Summary Findings directly above the Counts subsection; the standalone Vector Space Visualization section is dropped from the client-facing Doc entirely. | Gabe Jordan |
| 2026-08-19 | **Summary Findings opener removed (Gabe).** The AI-written opener paragraph ("This map captures...") no longer renders client-facing - Summary Findings goes straight to the chart and the Counts / Tier distribution / Localization subsections. Also resolves the fixed-vs-AI-prose question for that slot: there is no freeform opener anymore. | Gabe Jordan |
| 2026-08-19 | **Two-pillar outline (Gabe).** Client-facing Doc outline is now: What Is Entity Research? / Summary Findings (chart + counts) / Detailed Findings (tier tables, cluster architecture, bridge entities, localization - each demoted one heading level under it). Renderer inserts the Detailed Findings H2 automatically before the first detail section. | Gabe Jordan |
| 2026-08-19 | **Bridge Entities table moved (Gabe).** Renders directly below the Tier 3 table (before Cluster Architecture) in the client-facing Doc; renderer reorders automatically regardless of markdown order. | Gabe Jordan |
| 2026-08-19 | **Cluster Architecture diagram added (Gabe).** New `scripts/cluster-diagram.py` renders one bubble per cluster with its member entities inside (uniform text, exact TextPath width-fitting so labels never overflow), black bridge links with line weight = bridge strength, bubble border weight = average cluster weight, no per-entity size/color coding. Embeds in the Doc under Detailed Findings -> Cluster Architecture via the renderer's `--cluster-chart` arg. Regenerated per-folder from `entity-map.json` in the sweep. | Gabe Jordan |
| 2026-08-19 | **Detail order + section dividers (Gabe).** Detailed Findings order is now: Cluster Architecture (diagram first), Tier 1-3 tables, Bridge Entities below Tier 3, Localization last - renderer reorders regardless of markdown order. A thin CE-blue divider line renders between the H2 pillars (before Summary Findings and Detailed Findings). | Gabe Jordan |
