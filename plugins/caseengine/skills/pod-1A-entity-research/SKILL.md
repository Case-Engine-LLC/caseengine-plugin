...
- **`keyword-research.json`** - the keyword-research seed set from `pod-1B-keyword-research` at `templates [master]/AEO/Podcast/Keyword Research/{Practice Area}/{matching scope}/`. When present, PAA stacks seed candidate entity surface area. Optional signal overlay, not a required input.
