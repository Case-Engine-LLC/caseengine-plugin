...
1. **Load the contract** [deterministic] - read `references/introduction.md` for the four line jobs, the ten line 2 patterns, the eight credential frames and the six ask frames, and `references/statics.json` for the constant. Where the constant already covers the words, the constant wins and nothing regenerates them.
...
6. **Write line 4, the ask** [LLM] - carried in the payload as `segment_1.prompt` and the only bolded prompt in the document. One of the six frames, two sentences maximum, ending in a question mark with the ask last. Two asks, never three, and the second is always the real case in whatever wording fits: that clause is what earns a 15 to 30 minute answer instead of a two-minute explanation. No qualifier in the situation, present tense, and never ask for credentials again - line 3 did that. Do not pair a frame with a line 2 that already said the same thing. Then stop talking: no example to get them started, no narrowing, no "for instance".
...
- Line 2 carries `{{STATE}}` and never `{{CITY}}`, matches one of the approved patterns, and ends at its stakes clause (IN-3). Any number in it traces to a source recorded in `metadata.json`; no source means pattern A (IN-4).
- Line 3 carries exactly one credential frame (IN-5), lands on a number or a range, and is the only place `{{CITY}}` appears in S1, one city maximum.
- Line 4 is three short sentences in order: situation, ask, conditional story invitation. At most two asks, the second being the real case, and it need not end in a question mark (IN-6).
