...
- **Goal types and the selection rule the renderer applies in v2:**
  ...
  - **Narrative / story** - the prompt's real-case clause gets the weight; the interviewer's "walk me through one" is promoted from a fallback to a planned beat.
...
| `welcome_embedded` | none | Welcome back to the **{{PODCAST_NAME}}** Podcast. Embedded-name shows only (Gabe 2026-08-26). |
| `welcome_embedded_first` | none | Welcome to the **{{PODCAST_NAME}}** Podcast. Episode 1 of an embedded-name show. |
| `outro_note` | none | Keep it short. Thank them and mean it, sign off, then the reach-out. Do not recap the episode. |

- **Embedded-name shows (Gabe 2026-08-26).** When the podcast name embeds the attorney's name (e.g. "Car Accident Attorney w. Robert May"), line 1 takes the `welcome_embedded` / `welcome_embedded_first` variant - never "with **{{ATTORNEY_NAME}}**", which doubles the name. A "w." in the podcast name is spoken, and rendered in the welcome, as "with" (`statics.json -> spoken_forms`).
...
- **The setup is line 2, its own paragraph.** Straight into the tension, 25 to 30 words, no greeting of its own (the greeting is the STATIC welcome immediately above it). Built from one of the ten line-2 patterns in `references/introduction.md`, carrying `{{STATE}}` and never `{{CITY}}`.
...
The legacy twenty-question format produced answers that sounded like testimony. Every question scoped the answer, so nothing ran long enough to become a story, and the attorney's authority never got established because it was never asked for directly. v2 fixes three things at once: the line 3 credential establishes authority before anything is asked, the single prompt gives the attorney room to actually talk, and the prompt's real-case clause tells them what a good long answer contains so the room is not just empty. If a future edit adds a second prompt to Segment 1 "for safety", it has undone the whole thing.
